...
def _get_epsilons(pos_confs, neg_confs, delta):
    ...
    for i in range(num_examples):
        ...
        # Handle the special case of perfect separation
        fpr_fnr_both_zero = torch.eq(
            fpr + fnr, torch.zeros(total_num_thresholds).cuda()
        )
        tpr_tnr_both_ones = torch.eq(
        ...
        # Handle invalid thresholds (fpr or fnr = 0)
        thr_eps_is_zero = torch.eq(thr_eps, torch.zeros_like(thr_eps).cuda())
        fpr_or_fnr_is_zero = torch.eq(fpr * fnr, torch.zeros_like(thr_eps).cuda())
        thr_eps = torch.where(
            thr_eps_is_zero * fpr_or_fnr_is_zero,
        ...
        # Compute epsilon for valid thresholds
        safe_fpr = torch.clamp(1 - delta - fpr, min=1e-10)
        safe_fnr = torch.clamp(fnr, min=1e-10)
        thr_eps = torch.where(
            torch.eq(thr_eps, torch.zeros_like(thr_eps).cuda()),
            torch.log(safe_fpr) - torch.log(safe_fnr),
            thr_eps,
        )

        # Keep only valid thresholds
        thr_eps = thr_eps.detach().cpu().numpy()
        valid_inds = ~np.isnan(thr_eps)
        if not valid_inds.any():
            raise ValueError("No valid thresholds found, all epsilon values are NaN.")
        thr_eps = thr_eps[valid_inds]
        tpr = tpr.detach().cpu().numpy()[valid_inds]
        fpr = fpr.detach().cpu().numpy()[valid_inds]
        tnr = tnr.detach().cpu().numpy()[valid_inds]
        fnr = fnr.detach().cpu().numpy()[valid_inds]
        kept_thresholds = [thresholds_flat[ind] for ind in range(len(valid_inds)) if valid_inds[ind]]

        epsilons.append(np.nanmax(thr_eps))
        best_thresh = np.argmax(
            np.where(np.isnan(thr_eps), -np.inf, thr_eps)
        )
        all_fprs.append(np.array(fpr))
        all_tprs.append(np.array(tpr))
        all_fnrs.append(np.array(fnr))
