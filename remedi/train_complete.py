...
from remedi.KNIFE import KNIFE, fit_kernel
from remedi.REMEDI import REMEDI, train_model
...
def train_complete_model(args_data, args_REMEDI, args_KNIFE, base_dist_filename=None, seed=42, device='cpu',
    ...
    if args_data.type == 'triangle':
        train_dataset = TriangleDataset(args_data.n_samples, device, args_data.number, args_data.dim,
                                        args_data.shape_seed)
        test_dataset = TriangleDataset(args_data.n_samples, device, args_data.number, args_data.dim,
                                       args_data.shape_seed)
        train_loader = DataLoader(train_dataset, worker_init_fn=worker_init_fn, batch_size=args_KNIFE.batchsize,
                                  shuffle=True)
    ...
    if hasattr(train_dataset, 'entropy'):
        assert train_dataset.entropy() == test_dataset.entropy(), (f"Train and test dataset entropies do not match ("
                                                                   f"train: {train_dataset.entropy()}, "
                                                                   f"test: {test_dataset.entropy()})")
    ...
    ###################### VAE ################
    if vae:
        vae_model = VanillaVAE(args_data.dim, 32, [128, 64]).to(device)
        optimizer = torch.optim.Adam(vae_model.parameters(), lr=0.001)
        train_vae(train_loader, vae_model, optimizer, device=device)
        vae_entropy = entropy(vae_model, train_loader)
        print("#########################VAE ESTIMATED ENTROPY: {}######################".format(vae_entropy))
    ###################### VAE ################
    ...
    mini = REMEDI(base_dist, args_REMEDI.train_base_dist, args_REMEDI.hidden_dim, args_REMEDI.one_hot_y,
                  args_REMEDI.output_type).to(device)
    ...
    if args_REMEDI.train_base_dist:
        optimizer.add_param_group(
            {'params': mini.base_dist.parameters(), 'lr': args_REMEDI.lr_base_dist, 'weight_decay': 0})

    # Recreate data loaders, with REMEDI parameters
    train_loader = DataLoader(train_dataset, worker_init_fn=worker_init_fn, batch_size=args_REMEDI.batchsize,
                              shuffle=True)
...
if __name__ == "__main__":
    log_parent_dir = Path.cwd() / 'runs' / 'train_complete_testing' / (
            "test-" + datetime.now().strftime('%Y-%m-%d_%H-%M-%S'))
    log_parent_dir.mkdir(parents=True, exist_ok=True)

    # Train model
    remedi, train_losses, val_losses = train_complete_model(args_data=args_data, args_REMEDI=args_REMEDI,
                                                            args_KNIFE=args_KNIFE, base_dist_filename=None,
                                                            device=device, log_parent_dir=log_parent_dir)
