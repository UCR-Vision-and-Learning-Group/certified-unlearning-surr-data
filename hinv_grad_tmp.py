...
from src.data import get_train_test_datasets
...
def batched_hvp(model, dataloader, criterion, params, vector, device):
    ...
    for X_batch, y_batch in tqdm(dataloader, desc="Computing HVP"):
        ...
        hvp_batch = grad(grad_vector, params, grad_outputs=vector, retain_graph=True)
...
def main():
    ...
    train_dataset, test_dataset = get_train_test_datasets('caltech256-act',
                                                         train_path='./data/caltech256_linear_resnet18_train.pth',
                                                         test_path='./data/caltech256_linear_resnet18_test.pth')
    ...
    h_inv_grad = (len(train_dataset) * htrain_inv_grad - len(forget_dataset) * hforget_inv_grad) / (len(train_dataset) - len(forget_dataset))
    model = model.to('cpu')

    # Clone and update the model
    updated_model = nn.Linear(512, 257)
